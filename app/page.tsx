      ...
      <div className="relative flex items-center justify-center">
        ...
        <div className="core-420">
          <div className="text-black font-black text-center leading-none">
            ...
            <div className="text-4xl mt-1 font-bold">V10.1</div>
      ...
      <div className="mt-8 flex gap-3">
        {['CORE','TRON','MAGMORE'].map(m=>(
          <button key={m} onClick={()=>setMode(m)}
            className={`px-6 py-2.5 rounded-full text-[12px] tracking-[0.2em] border transition-all ${mode===m?'bg-white text-black border-white shadow-[0_0_20px_rgba(255,255,255,0.5)]':'border-white/20 text-white/60 hover:border-white/40 hover:text-white/80'}`}>
            {m}
          </button>
        ))}
      </div>

      <p className="mt-8 text-[10px] tracking-[0.3em] opacity-30">
        ENTER THE CORE — {mode} MODE
      </p>
    </main>
  )
}
